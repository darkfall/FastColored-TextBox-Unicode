...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter: IDisposable
    {
        ...
        public virtual void ClearDescriptionFile()
        {
            descByXMLfileNames.Clear();
        }
    ...
    brackets.Attributes["left2"].Value == "" || brackets.Attributes["right2"].Value == "")
        ...
        Regex HTMLTagRegex, HTMLTagNameRegex, HTMLEndTagRegex, HTMLAttrRegex, HTMLAttrValRegex, HTMLCommentRegex1, HTMLCommentRegex2;
        ...
        void InitHTMLRegex()
        {
            ...
            HTMLAttrRegex = new Regex(@"(?<range>\S+?)='[^']*'|(?<range>\S+)=""[^""]*""|(?<range>\S+)=\S+", RegexCompiledOption);
            HTMLAttrValRegex = new Regex(@"\S+?=(?<range>'[^']*')|\S+=(?<range>""[^""]*"")|\S+=(?<range>\S+)", RegexCompiledOption);
        }
