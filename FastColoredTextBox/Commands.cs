            ...
            switch (c)
            {
                ...
                case '\t':
                    for (int i = 0; i < tb.TabLength;i++ )
                        ts[tb.Selection.Start.iLine].Insert(tb.Selection.Start.iChar, new Char(' ', FastColoredTextBox.GetCharSize(tb.Font, ' ')));
                    tb.Selection.Start = new Place(tb.Selection.Start.iChar + tb.TabLength, tb.Selection.Start.iLine);
                    break;
                default:
                    ts[tb.Selection.Start.iLine].Insert(tb.Selection.Start.iChar, new Char(c, FastColoredTextBox.GetCharSize(tb.Font, ' ')));
                    tb.Selection.Start = new Place(tb.Selection.Start.iChar + 1, tb.Selection.Start.iLine);
                    break;
            }
        }
